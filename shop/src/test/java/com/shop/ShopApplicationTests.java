...
/*	@Test
	...
	public void createItemList() {
		for (int i = 1; i <= 10; i++) {
			...
			item.setItemDetail("테스트 상품입니다." + i);
			item.setItemSellStatus(ItemSellStatus.SELL);
			item.setStockNumber(100);
			item.setRegTime(LocalDateTime.now());
			item.setUpdateTime(LocalDateTime.now());
	...
	@Test
	@DisplayName("상품명 조회 or 조건")
	public void findByItemNmOrItemDetailTest() {
		this.createItemList();
		List<Item> itemList =
				itemRepository.findByItemNmOrItemDetail("테스트상품1", "테스트 상품입니다.5");
		for(Item item : itemList) {
			System.out.println(item.toString());
		}
	}
	...
	@Test
	@DisplayName("@Query 이용한 조회 테스트")
	public void findByItemDetailTest() {
		this.createItemList();
		List<Item> itemList = itemRepository.findByItemDetail("테스트 상품입니다.");
		for(Item item : itemList) {
			System.out.println(item.toString());
		}
	}
	...
	public void queryDslTest() {
		...
		JPAQuery<Item> query = queryFactory.selectFrom(qItem)
				...
				.where(qItem.itemDetail.like("%" + "테스트 상품입니다." + "%"))
